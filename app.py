...
with tab1:
    ...
    if pwd == "textilia2026":
        ...
        with col_p:
            st.subheader("🚀 Snabb-import & Ruttoptimering")
            ...
            if st.button("⚡ Optimera Körordning"):
                temp_stopp = []
    ...
    if rutter:
        ...
        for i, s in enumerate(mina):
            with st.expander(f"STOPP {i+1}: {s['adress']}", expanded=(i==0)):
                if s['prio'] == 1: st.warning("⭐ Denna adress har högsta prioritet!")
                st.write(f"Väntad retur: **{s['vagnar']} vagnar**")
                
                st.link_button("🧭 Starta Navigering", f"https://www.google.com/maps/search/?api=1&query={urllib.parse.quote(s['adress'] + ', Göteborg')}")
                ...
                if c2.button("⚠️ Problem", key=f"err_{s['id']}"):
                    st.camera_input("Ta bild på hinder", key=f"cam_{s['id']}")
...
with tab3:
    ...
    if st.session_state.rutten:
        ...
        st.info("Ingen data att visa än.")
